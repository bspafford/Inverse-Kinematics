...
public class bully1 : MonoBehaviour {
    ...
    void Correct() {
        float3 dq = SolveIKStep(new float2(desiredLoc.x, desiredLoc.y), arm1.localEulerAngles.z, arm2.localEulerAngles.z, arm3.localEulerAngles.z, arm1Length, arm2Length, arm3Length);

        arm1.localRotation = Quaternion.Euler(0f, 0f, (arm1.localEulerAngles.z + dq.x * Time.deltaTime * speed) * Mathf.Rad2Deg);
        arm2.localRotation = Quaternion.Euler(0f, 0f, (arm2.localEulerAngles.z + dq.y * Time.deltaTime * speed) * Mathf.Rad2Deg);
        arm3.localRotation = Quaternion.Euler(0f, 0f, (arm3.localEulerAngles.z + dq.z * Time.deltaTime * speed) * Mathf.Rad2Deg);
    }
